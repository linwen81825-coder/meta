...
#
# 说明：
# top_k=1 时，就是 hard top1 routing，并使用 Straight-Through gate
...
class Expert(nn.Module):
    ...
    def __init__(
        ...
        self.net = nn.Sequential(
            # 第一层：把 ResNet18 输出的 512 维特征
            # 映射到更大的 expert hidden 空间
            nn.Linear(
                feature_dim,
                hidden_dim,
            ),
...
class SwitchMoEHead(nn.Module):
    ...
    top_k=1：
    每个样本只选择 1 个 expert。
    前向是严格 hard top1，gate 的数值等于 1；
    反向通过 Straight-Through gate 让分类损失能够训练 router。
    ...
    def forward(
        ...
        if self.top_k == 1:
            # Top-1 前向保持严格 hard routing：
            # 被选中 expert 的 gate 数值恒等于 1。
            #
            # Straight-Through 写法：
            #
            # forward:
            ...
            # 归一化后是 [0.75, 0.25]
            #
            # 被选中的 top-k expert 的 gate 加起来等于 1。
            topk_gates = (
                topk_probs
                / topk_probs.sum(
                    dim=-1,
        ...
        # 为了兼容原来的日志和统计逻辑，
        # 仍然保留 top1 信息
        top1_probs = topk_probs[
            :,
            0,
        ]
        # [B]

        top1_indices = topk_indices[
            :,
            0,
        ]
        # [B]
        ...
        for k_id in range(
            ...
            for (
                ...
                # 注意这里是 +=
                # 因为 top_k=2 时，一个样本会有两个 expert 输出，
                # 需要相加
                final_logits[
                    mask
                ] += expert_logits
...
class ResNet18SwitchMoE(nn.Module):
    ...
    def __init__(
        ...
        # 原版 ResNet18 第一层是 7x7 conv + stride=2，
        # 不太适合 CIFAR10
